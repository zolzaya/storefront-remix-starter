...
export default function ProductGallery({ assets }: ProductGalleryProps) {
    ...
    <div className="relative flex w-full max-h-[600px] aspect-[4/3]">
      ...
      >
        {assets.map((asset, index, thumbsArray) => (
          ...
          >
            <img className="border border-neutral-200" width="78" height="78" src={asset.preview} />
          </button>
        ))}
      </SfScrollable>
      ...
      >
        {assets.map((asset, index) => (
          <div key={`${asset.id}-${index}`} className="flex justify-center h-full basis-full shrink-0 grow snap-center">
            <img
